...
| 新增页面 | 「在业务分包里新增『订单详情』页面，并补齐路由入口。」 | 识别为 Feature 的 add-page 场景；确定归属目录与路由注册方式；按编码约定实现；路由 / 依赖变更跑 build、源码跑 lint，组件索引存在时登记。 |
...
| 视觉 Token 梳理 | 「梳理项目里的颜色、字号、间距、圆角和阴影 Token，先给治理方案。」 | 按「调查 / 分析」路由只读盘点（无专门 Skill）；盘点定义与实际用量；识别重复值、近似值和命名不一致；给出推荐 Token 与旧值映射；未经确认不批量替换代码。 |
| 组件 / 模块复用治理 | 「我要把这个弹窗抽成公共组件，帮我判断该不该公共化并更新索引。」 | 先扫描真实调用位置，按「仅 1 处私有 / 模块内共享 / ≥2 模块公共化」给建议；公共化是改动，说明依据并先确认；复用索引存在时更新名称、位置、说明与使用位置。 |
| 旧项目功能迁移 | 「把旧仓库的『优惠券』功能迁到当前项目，先评估再动手。」 | 识别为迁移；先做迁移预检并确认范围；对旧代码逐条取证并标 reuse / migrate / replace / out_of_scope；实现最小迁移，改公共契约先确认；完成后做行为核验与等价审查。 |
| 自动化测试 | 「给项目搭一套单元测试 + E2E 的分层测试门禁，先出方案。」 | 按「测试分层 → runner 封装 → 统一报告」搭门禁；复用现有语言运行时、包管理器和测试框架；统一 PASS / FAIL / STALE / NOT RUN 语义，不把旧结果伪装成通过。 |
...
> 表中「组件 / 模块复用治理」「旧项目功能迁移」「自动化测试」依赖可选能力，「新增页面」是按需 Skill，仅当对应能力已安装（或任务首次出现且安装来源可用）时才具备完整行为；未安装时 AI 按基础路由说明缺失，不视为 BLOCK。
